...
class CustomBatchNormAutograd(nn.Module):
  ...
  def forward(self, input):
    ...
    if input.shape[1] != self.n_neurons:
        raise ValueError("input.shape[1] must be of shape", self.n_neurons)
...
class CustomBatchNormManualFunction(torch.autograd.Function):
  ...
  def forward(ctx, input, gamma, beta, eps=1e-5):
    ...
    ctx.save_for_backward(normalized, variance, gamma)
    ctx.eps = eps
  ...
  def backward(ctx, grad_output):
    ...
    normalized, variance, gamma = ctx.saved_variables
    eps = ctx.eps

    grad_normalized = grad_output * gamma
    size = grad_output.shape[0]

    grad_input = (size * grad_normalized - grad_normalized.sum(dim=0) -
                  normalized * torch.sum(grad_normalized * normalized, dim=0)) / (size * torch.sqrt(variance + eps))
...
class CustomBatchNormManualModule(nn.Module):
  ...
  def forward(self, input):
    ...
    f = CustomBatchNormManualFunction()
    out = f.apply(input, self.gamma, self.beta, self.eps)
